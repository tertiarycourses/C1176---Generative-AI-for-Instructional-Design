...
def activity_overview(tag,title,desc,build,services,kicker):
    ...
    dL=len(str(desc)); dsize = 21 if dL<=200 else (17 if dL<=320 else (15 if dL<=560 else (14 if dL<=760 else 13)))
    txt(s,Inches(0.85),Inches(2.55),Inches(11.7),Inches(1.65),[[(desc,dsize,_ink(),False)]])
    rect(s,Inches(0.85),Inches(4.35),Inches(11.7),Inches(2.15),_panel())
    txt(s,Inches(1.1),Inches(4.5),Inches(11),Inches(0.4),[[("You'll build",14,_acc(BLUE),True)]])
    # Auto-fit the build line too, so a long deliverable never clips into Tools.
    bL=len(str(build)); bsize = 18 if bL<=180 else (16 if bL<=280 else 15)
    txt(s,Inches(1.1),Inches(4.88),Inches(11),Inches(0.95),[[(build,bsize,_ink(),True)]])
    txt(s,Inches(1.1),Inches(5.9),Inches(11.2),Inches(0.6),[[("Tools:  ",13,_grey(),True),(services,13,_grey(),False)]]); footer(s); return s
